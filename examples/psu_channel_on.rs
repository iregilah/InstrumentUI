...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if ip_addr.is_none() {
        ...
        if let Some((_, info)) = devices.iter().find(|(_, info)| info.instrument_type.as_deref() == Some("Power Supply") || info.model.as_deref().map(|m| m.starts_with("DP")) == Some(true)) {
    ...
    let curr = 0.5;
    if use_usb {
        send_scpi_via_usb(USB_VID, USB_PID, &format!(":SOUR{ch}:VOLT {volt}"))?;
        send_scpi_via_usb(USB_VID, USB_PID, &format!(":SOUR{ch}:CURR {curr}"))?;
        send_scpi_via_usb(USB_VID, USB_PID, &format!(":OUTP CH{ch},ON"))?;
