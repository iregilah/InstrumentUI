...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if use_usb {
        ...
        send_scpi_via_usb(USB_VID, USB_PID, ":LA:STAT ON")?;
        println!(">> LA ON – színes D0…D15 jelek a képernyő alján.");
        thread::sleep(Duration::from_secs(1));

        send_scpi_via_usb(USB_VID, USB_PID, ":LA:POD1:THR 1")?;
        send_scpi_via_usb(USB_VID, USB_PID, ":LA:POD2:THR 1")?;
        println!(">> Threshold 1 V – trigger‑/dekóder‑szinten is frissül.");
        thread::sleep(Duration::from_secs(1));

        for d in 8..16 {
            send_scpi_via_usb(USB_VID, USB_PID, &format!(":LA:DIG{d}:DISP OFF"))?;
            println!("   D{d} OFF");
            thread::sleep(Duration::from_millis(400));
        }

        send_scpi_via_usb(USB_VID, USB_PID, ":LA:STAT OFF")?;
        println!("=== DEMO END ===");
        return Ok(());
    }
